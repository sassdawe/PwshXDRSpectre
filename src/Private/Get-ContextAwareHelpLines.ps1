function Get-ContextAwareHelpLines {
    ...
    if ($IsQueryMode.IsPresent) {
        switch ($ActivePanel) {
            'incidents' { return @('↑/↓ query catalog | Enter execute selected query | Alt+X execute selected query | Alt+H return to incident workflow | Alt+K toggle input debug | F1 help | Tab or PgUp/PgDn switch | Ctrl+C exit') }
            'incident_details' { return @('Query preview | Enter or Alt+X execute selected query | Alt+H return to incident workflow | Alt+K toggle input debug | Tab or PgUp/PgDn switch | F1 help | Ctrl+C exit') }
            'alerts' { return @('Recent query runs | Alt+K toggle input debug | Tab or PgUp/PgDn switch | Alt+H return to incident workflow | F1 help | Ctrl+C exit') }
            'alert_details' { return @('Query results for selected catalog entry | Alt+X rerun selected query | Alt+H return to incident workflow | Alt+K toggle input debug | F1 help | Ctrl+C exit') }
            'action_status' { return @('↑/↓ select query action | Enter execute selected action | Alt+X run selected query | Alt+H return to incident workflow | Alt+K toggle input debug | F1 help | Tab or PgUp/PgDn switch | Ctrl+C exit') }
        }

        return @('Hunting mode | Enter or Alt+X execute selected query | Alt+H return to incident workflow | Alt+K toggle input debug | F1 help | Tab or PgUp/PgDn switch | Ctrl+C exit')
    }
